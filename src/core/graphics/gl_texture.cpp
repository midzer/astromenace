                          ...
                          eTextureCompressionType CompressionType)
{
    ...
    case eTextureCompressionType::BPTC:
        if (!vw_DevCaps().ARB_texture_compression_bptc && !vw_DevCaps().OpenGL_4_2_supported) {
            return 0;
        }
        if (Bytes == 4) {
    ...
    if (MipMap) {
        ...
        if (pfn_glGenerateMipmap && pfn_glTexStorage2D) {
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, Width, Height, Format, GL_UNSIGNED_BYTE, PixelsArray.get());
            pfn_glGenerateMipmap(GL_TEXTURE_2D);
        } else if (pfn_glGenerateMipmap) {
            glTexImage2D(GL_TEXTURE_2D, 0, InternalFormat, Width, Height, 0, Format, GL_UNSIGNED_BYTE, PixelsArray.get());
            pfn_glGenerateMipmap(GL_TEXTURE_2D);
        } else if (vw_DevCaps().SGIS_generate_mipmap) {
            glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP_SGIS, GL_TRUE);
            glTexImage2D(GL_TEXTURE_2D, 0, InternalFormat, Width, Height, 0, Format, GL_UNSIGNED_BYTE, PixelsArray.get());
        } else {
            gluBuild2DMipmaps(GL_TEXTURE_2D, InternalFormat, Width, Height, Format, GL_UNSIGNED_BYTE, PixelsArray.get());
        }
    } else {
